...
async function recebetempo() {
    ...
    var datAccTempo = [];
    const { data } = await axios.get("https://api.weather.com/v1/geocode/-21.129/-41.677/forecast/hourly/48hour.json?units=m&language=pt-BR&apiKey=320c9252a6e642f38c9252a6e682f3c6");
...
async function recebeDados() {
    ...
    for (var i = 0; i < result.temp.length; i++) {
        ...
        dados[i] = {'temp':parseFloat(resultT.toFixed(1)),'press':parseFloat(resultP.toFixed(1)), 'humid':parseFloat(resultU.toFixed(1))}
        valores[i] = {'data':result.temp[i][0],'temp':parseFloat(result.temp[i][1]),
                      'press':parseFloat(result.press[i][1]), 'humid':parseFloat(result.humid[i][1])}
    }
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        dados2[i]={'input':{'temp':v2,'press':v3, 'humid':v4}, 'output':{'chuva':v1}}
    }
...
async function redeneural() {
    ...
    const final = {'regressao':valores, 'previsao':output,'previsao2':await recebetempo()}
