...
async function recebeEstacao() {
    ...
    for (var i = 0; i < 48; i++) {
        ...
        let v4 = parseFloat(data[i].moisture);
        datEstT[i2] = [v1,v2];
        datEstP[i2] = [v1,v3];
        datEstU[i2] = [v1,v4];
        i2++;
...
async function retornaData() {
    const valor = await regressao();
    var valorT = valor[0];
    var valorP = valor[1];
    var valorU = valor[2];
    var valor2 = await concatenar();
    ...
    var Temp = [];
    for (var i = 48; i < 60; i++) {
        const date = new Date(valor2D[i][0]-10800000);
